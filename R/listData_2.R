listTables <- function(dirIn, maxLevel = 1, delimiter = "$", tmpdir = tempdir()){
...
resolveRow <- function(tableIn, rowIn, tmpdir = tempdir()){
    ...
    #   temporary location.
    if(length(grep("(?i)\\.zip$", tableIn[rowIn, "tempDsn"]))){
        
        dirOut <- tempfile(pattern = "temp_extract_", tmpdir = tmpdir)
        
        cat("\n", dirOut, "\n\n")
        
        toUnzip <- unzip(tableOut[rowIn, "tempDsn"], list = TRUE)$Name
        toUnzip <- grep(spatialTables(c("table", "tempDsn")), toUnzip, value = TRUE)
        
        unzip(tableOut[rowIn, "tempDsn"], files = toUnzip, exdir = dirOut)
        
        #Update the tempDsn to represent the folder it's being stored in.
        sharedSource <- tableOut[, "tempDsn"] == tableOut[rowIn, "tempDsn"]
        sharedSource <- sharedSource & !is.na(sharedSource)
        
        tableOut[sharedSource, "tempDsn"] <- dirOut
    ...
    if(!is.na(tableOut[rowIn, "tempDsn"])){
        pathUse <- file.path(tableOut[rowIn, "tempDsn"], tableOut[rowIn, "dsn"])
    } else {
    ...
    if(length(tablesOut) > 0){
        tableUse <- data.frame(dsn = tablesOut,
                               type = "table",
                               container = delimitPath(tableOut[rowIn, "container"],
                                                       tableOut[rowIn, "dsn"]
                                                       ),
                               tempDsn = delimitPath(tableOut[rowIn, "tempDsn"],
                                                     tableOut[rowIn, "dsn"]
                                                     ),
...
delimitPath <- function(currentPath, newPath, delimiter = "$"){
    if(is.na(currentPath)){
        ...
        if(length(grep(spatialTables("container"), currentPath, invert = TRUE))){
